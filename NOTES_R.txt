Some miscellaneous notes about R.
---------------------------------
  ...
  2. Customize Sweave for better output.
     ...
     NOTE: If you're using TeXLive then you might have to add Sweave to the local
           ...
           If it is missing do the following:
     ...
     c. Avoid comment loss and code reformatting, and set plot size (default is 6x6)
  ...
  25. #define USE_RINTERNALS before #include <Rinternals.h> is needed to gain direct
      access to internals of SEXPRECS.  See last paragraph of section 1.7 of R
      Internals.
  ...
  29. Use `log1p` when you want to calculate log(1+x) and |x| << 1.  I.e., if
  ...
  32. Fix some parameters for plotting.  This was suggested by Andrew Gelman.  See
      ...
      NOTE:  This causes problem with plotting in other packages.  `raster` is a good
  ...
  37. 'format', 'as.character', and 'strftime' convert from '"POSIXlt"' and
      ...
      Mnemonic:  strptime  <==> string parse time  i.e. character -> date/time class
 ...
 39. If you want to split a column into multiple columns in data.table using strsplit
     can be confusing and slow.  Instead use tstrsplit provided by data.table.
 ...
 49. You can use zApply or stackApply to group bunch of layers (say based on
 ...
 60. If there is problem installing rgl then just download the binary
     ...
     where you put the "tgz" like this:
       ...
       shell> Rscript -e "install.packages('rgl_0.96.0.tgz',repos=NULL)"
     OR
 ...
 61. If there is problem with installing gmp/Rmpfr (especially when using R
     ...
     it quite easily with the following:
       ...
       shell> R CMD config --all  # print all environment variables!
       shell> R CMD config CFLAGS # will give the value of CFLAGS from /opt/local
 ...
 64. Use `edit' to modify function definition.  ?edit
     R> f <- function(x) x * x
 ...
 91. Rmpi needs to be installed from source and if you're using openmpi from macports then
     you need to use configure.args to set variables needed to get Rmpi to compile.  I had to
     use the following:
       ...
       shell$ port contents openmpi-devel-default # find paths for Rmpi-libpath and Rmpi-include!
